...
def main():
    ...
    if "JBMono" in svg:
        print(f"{target}: already carries the font — nothing to do")
        return
    ...
    if "<style>" not in svg:
        raise SystemExit(f"{target}: no <style> block found")

    svg = svg.replace("<style>", f"<style>{rule}", 1)

    swapped, n = re.subn(r'font-family="[^"]*"', f'font-family="{FAMILY}"', svg)
    if not n:
        raise SystemExit(f"{target}: no font-family attribute to replace")
